...
#define JOYSTICK_SAMPLES                                                       \
  5 /**< Number of samples to average for noise reduction. */
...
public:
   ...
   */
  Joystick(uint8_t pin_x, uint8_t pin_y, uint8_t pin_z,
           int deadzone = JOYSTICK_DEADZONE)
      : _pin_x(pin_x), _pin_y(pin_y), _pin_z(pin_z), _deadzone(deadzone),
        _center_x(0), _center_y(0), _last_btn_state(HIGH),
        _last_debounce_time(0){};

  /**
   * @brief Initializes hardware pins and performs auto-calibration of the
   * center point.
   * @details
  ...
  int get_speed_x(int min_out = -255, int max_out = 255) {
    return _map_speed(get_x(), min_out, max_out);
  }
  int get_speed_y(int min_out = -255, int max_out = 255) {
    return _map_speed(get_y(), min_out, max_out);
  }
