...
// TODO: add detecition algorithm corrently half implemented
std::tuple<MoveDirectionX, MoveDirectionY>
CameraDiffDetection::detect_object(camera_fb_t *frame) {
  /*
...
void CameraDiffDetection::roberts_cross(camera_fb_t *frame,
                                        uint8_t *output_edges) {
  int w = frame->width;
  int h = frame->height;

  uint16_t *rgb_buf = (uint16_t *)heap_caps_malloc(
      w * h * 2, MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT);
  ...
  if (jpg2rgb565(frame->buf, frame->len, (uint8_t *)rgb_buf, JPG_SCALE_NONE)) {
    for (int y = 0; y < h - 1; y++) {
      ...
      for (int x = 0; x < w - 1; x++) {
        ...
        output_edges[curr_row + x] =
            (magnitude > 255) ? 255 : (uint8_t)magnitude;
      }
    }
  }
...
uint8_t CameraDiffDetection::rgb565_to_greyscale(uint16_t pixel) {
  // Extract R (5 bits), G (6 bits), G (5 bits) and move bits to a gretscale
  // format
